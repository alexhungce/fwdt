...
static DEVICE_ATTR(acpi_arg0, S_IRUGO | S_IWUSR, acpi_arg0_read,
		   acpi_arg0_write);
static DEVICE_ATTR(acpi_arg1, S_IRUGO | S_IWUSR, acpi_arg1_read,
		   acpi_arg1_write);
static DEVICE_ATTR(acpi_method_0_1, S_IRUGO | S_IWUSR, acpi_method_0_1_read,
		   acpi_method_name_write);
static DEVICE_ATTR(acpi_method_1_0, S_IRUGO | S_IWUSR, acpi_method_1_0_read,
		   acpi_method_name_write);
static DEVICE_ATTR(acpi_method_1_1, S_IRUGO | S_IWUSR, acpi_method_1_1_read,
		   acpi_method_name_write);
static DEVICE_ATTR(acpi_method_2_0, S_IRUGO | S_IWUSR, acpi_method_2_0_read,
		   acpi_method_name_write);
static DEVICE_ATTR(acpi_method_2_1, S_IRUGO | S_IWUSR, acpi_method_2_1_read,
		   acpi_method_name_write);
...
/* Memory */
static DEVICE_ATTR(mem_address, S_IRUGO | S_IWUSR, mem_read_address,
		   mem_write_address);
...
/* I/O */
static DEVICE_ATTR(io_address, S_IRUGO | S_IWUSR, io_read_address,
		   io_write_address);
...
static DEVICE_ATTR(pci_reg, S_IRUGO | S_IWUSR, pci_read_offset,
		   pci_write_offset);
...
static struct attribute *fwdt_acpi_sysfs_entries[] = {
    ...
    &dev_attr_acpi_method_2_1.attr,
    &dev_attr_acpi_method_2_0.attr,
...
static struct attribute *fwdt_acpi_ec_sysfs_entries[] = {
    &dev_attr_ec_address.attr, &dev_attr_ec_data.attr,
    &dev_attr_ec_qmethod.attr, NULL,
};
...
static struct attribute *fwdt_memory_sysfs_entries[] = {
    &dev_attr_mem_address.attr, &dev_attr_mem_data.attr, NULL,
};
...
static struct attribute *fwdt_io_sysfs_entries[] = {
    &dev_attr_io_address.attr, &dev_attr_iow_data.attr, &dev_attr_iob_data.attr,
    NULL,
};
...
static struct attribute *fwdt_pci_sysfs_entries[] = {
    &dev_attr_pci_id.attr, &dev_attr_pci_reg.attr, &dev_attr_pci_data.attr,
    NULL,
};
...
static struct miscdevice fwdt_runtime_dev = {MISC_DYNAMIC_MINOR, "fwdt",
					     &fwdt_runtime_fops};
...
static int fwdt_setup(struct platform_device *device)
{
	...
	status = acpi_get_devices("PNP0C09", acpi_handle_locate_callback, NULL,
				  &ec_device);
